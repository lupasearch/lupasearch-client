...
describe("SearchResults", () => {
  ...
  it("should correctly display didYouMean", () => {
    ...
    cy.get(searchResultsDidYouMeanTextLabel)
      .should("exist")
      .should("contain.text", "Did you mean to search: shoes meant");
  });
  ...
  const checkParentChildrenCount = (
    parent: string,
    children: string,
    count: number
  ) => {
  ...
  it("should show correct number of products when changing page size", () => {
    ...
    cy.get(searchResultsPageSize)
      .find(searchResultsSelectDropdown)
      .last()
      .select("24");
