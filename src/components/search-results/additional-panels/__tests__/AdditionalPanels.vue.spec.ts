...
describe("AdditionalPanels", () => {
  it("should not render component if there are no additional panels configured", () => {
    ...
    expect(wrapper.find(".lupa-results-additional-panels").exists()).toBe(
      false
    );
  });
  ...
  it("should render component if there are additional panels configured", () => {
    const wrapper = getComponent({
      additionalPanels: [
        validPanel("1") as SearchResultsAdditionalPanelOptions,
      ],
    });
  ...
  it("should render all panels", () => {
    ...
    expect(wrapper.findAllComponents(AdditionalPanel).wrappers.length).toBe(4);
  });
});
