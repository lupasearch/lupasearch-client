...
describe("hierarchyFacet", () => {
  ...
  it("should display multi level value", () => {
    const wrapper = getComponent();
    expect(
      wrapper.find(".lupa-facet-hierarchy .lupa-facet-hierarchy").text()
    ).toBe("two-two");
  });
  ...
  it("should have a checked value if facet key is in current filters", () => {
    ...
    expect(
      facet0.find(".lupa-term-checkbox").classes().includes("checked")
    ).toBe(false);
    const facet5 = facets.at(4);
    expect(facet5.text()).toBe("three");
    expect(
      facet5.find(".lupa-term-checkbox").classes().includes("checked")
    ).toBe(true);

    const facet2 = facets.at(2);
    expect(facet2.text()).toBe("two-two");
    expect(
      facet2.find(".lupa-term-checkbox").classes().includes("checked")
    ).toBe(true);
  });
  ...
  it("should emit select event on click", () => {
    ...
    expect(wrapper.emitted().select).toEqual([
      [{ key: "price", type: "hierarchy", value: "one" }],
    ]);
  });
});
