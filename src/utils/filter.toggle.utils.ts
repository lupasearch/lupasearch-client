...
export const toggleHierarchyFilter = (
  ...
  const currentFilter = currentFilters?.[
    facetAction.key
  ] as FilterGroupItemTypeHierarchy;
  ...
  appendParams({
    params: [
      getFacetParam(facetAction.key, newParams, FACET_PARAMS_TYPE.HIERARCHY),
    ],
    paramsToRemove: [QUERY_PARAMS.PAGE],
  });
};
  ...
  if (params?.some((p) => p.startsWith(param))) {
    return removeAllLevels
      ? getMostSpecificHierarchyTerms(
          params.filter((p) => !p.startsWith(param))
        )
