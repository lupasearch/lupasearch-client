...
describe("retrieveHistory", () => {
  ...
  it("should return empty array if local storage is unavailable", () => {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    window.localStorage = jest.fn().mockImplementationOnce(() => {
      throw "forbidden";
    });
    const history = retrieveHistory();
    expect(history).toEqual([]);
...
describe("saveHistory", () => {
  ...
  it("should not throw exception if local storage is unavailable", () => {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    window.localStorage = jest.fn().mockImplementationOnce(() => {
      throw "forbidden";
  ...
  it("should limit a number of saved items", () => {
    saveHistory(
      Array.from(Array(HISTORY_MAX_ITEMS + 5).keys()) as unknown as string[]
    );
    expect(window.localStorage.getItem(HISTORY_LOCAL_STORAGE_KEY)).toBe(
      `[0,1,2,3,4,5,6]`
    );
